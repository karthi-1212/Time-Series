...
# give the chart file name ->png(file = "rainfall.png")
...
# convert to matrix
combined_rainfall <- matrix(c(rainfall_1,rainfall_2),nrow = 12);combined_rainfall

# Converting to time series 
combined_rainfall_timeseries <- ts(combined_rainfall, start = c(2012,1), frequency = 12);combined_rainfall_timeseries

#png(file = "rainfall_combined.png")


plot(combined_rainfall_timeseries)


# AIR PASSENGERS
data=AirPassengers;data
data("AirPassengers")
...
ts_data <- ts(AirPassengers, frequency = 12);ts_data
d_data <- decompose(ts_data,"multiplicative");d_data
plot(d_data)
plot(d_data$trend)
plot(d_data$seasonal)
plot(d_data$random)
...
plot(X, main = "Random walk process")
